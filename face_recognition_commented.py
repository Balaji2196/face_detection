...
def detect(gray,frame):  # We create a function that takes as input the image in black and white (gray) and the original image (frame), and that will return the same image with the detector rectangles.
    ...
    for (x, y, w, h) in faces:  # For each detected face:
        ...
        cv2.putText(frame, 'face', (x, y), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 1, cv2.LINE_AA)
        ...
        for (ex, ey, ew, eh) in eyes:  # For each detected eye:
            ...
            cv2.putText(roi_color, 'eyes', (ex, ey), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 1, cv2.LINE_AA)
        smiles = smile_cascade.detectMultiScale(roi_gray, 1.6, 22)
        for (sx, sy, sw, sh) in smiles:
            cv2.rectangle(roi_color, (sx, sy), (sx + sw, sy + sh), (0, 0, 255), 2)
            cv2.putText(roi_color, 'smile', (sx, sy), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 1, cv2.LINE_AA)
